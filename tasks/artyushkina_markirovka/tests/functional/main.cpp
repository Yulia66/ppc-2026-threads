...
class ArtyushkinaMarkirovkaFuncTestsTBB : public ppc::util::BaseRunFuncTests<InType, OutType, TestType> {
 public:
  static std::string PrintTestParam(const TestType &test_param) {
    return "TBB_" + std::to_string(std::get<0>(test_param)) + "_" + std::get<1>(test_param);
  }
 ...
 protected:
  ...
  bool CheckTestOutputData(OutType &output_data) final {
    ...
    if (test_id == 5 || test_id == 6) {
      std::cout << "\n=== TBB INPUT MATRIX (test " << test_id << ") ===\n";
      PrintInputMatrix(input_data_);
      std::cout << "========================\n\n";
      PrintMatrix(output_data, "=== TBB OUTPUT LABELS ===\n");
    }

    if (output_data != expected_) {
      std::cout << "TBB Expected: ";
      for (auto val : expected_) {
        std::cout << static_cast<int>(val) << ' ';
      }
      std::cout << '\n';
