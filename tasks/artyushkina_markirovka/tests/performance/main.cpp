...
class ArtyushkinaMarkirovkaTBBPerfTests : public ppc::util::BaseRunPerfTests<InType, OutType> {
  ...
  bool CheckTestOutputData(OutType &output_data) final {
    ...
    if (std::cmp_not_equal(rows, input_data_[0]) || std::cmp_not_equal(cols, input_data_[1])) {
      std::cerr << "TBB Size mismatch: expected " << static_cast<int>(input_data_[0]) << "x"
                << static_cast<int>(input_data_[1]) << ", got " << rows << "x" << cols << '\n';
      return false;
    }
    ...
    for (int i = 0; i < rows; ++i) {
      for (int j = 0; j < cols; ++j) {
        ...
        if (input_data_[input_idx] == 0 && output_data[output_idx] == 0) {
          std::cerr << "TBB Object pixel at (" << i << "," << j << ") not labeled!\n";
          valid = false;
        }
        if (input_data_[input_idx] != 0 && output_data[output_idx] != 0) {
          std::cerr << "TBB Background pixel at (" << i << "," << j << ") incorrectly labeled!\n";
          valid = false;
        }
      }
    }
...
TEST_P(ArtyushkinaMarkirovkaTBBPerfTests, RunPerfModesTBB) {
  ExecuteTest(GetParam());
}
...
INSTANTIATE_TEST_SUITE_P(RunModeTestsTBB, ArtyushkinaMarkirovkaTBBPerfTests, kGtestValuesTBB, kPerfTestNameTBB);
